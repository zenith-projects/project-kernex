...
public partial class AxiaPanel : PanelContainer
{
    ...
    private static readonly Color ColorAxiaText = new(0.831f, 0.659f, 0.294f);
    private static readonly Color ColorDimText = new(0.45f, 0.45f, 0.48f);

    private AxiaAvatar _avatar;
    ...
    {
        _avatar = GetNode<AxiaAvatar>("MainLayout/HeaderBar/HeaderMargin/HeaderContent/AxiaAvatar");
    ...
    public void SetThinking(bool thinking)
    {
        ...
        if (thinking)
            _thinkingLabel.Text = "AXIA is thinking...";
    }
    ...
    public void AppendAxiaMessage(string message)
    {
        var labelHex = ColorGold.ToHtml(false);
        var textHex = ColorAxiaText.ToHtml(false);
        _chatLog.AppendText($"\n[color=#{labelHex}]AXIA[/color]\n");
        _chatLog.AppendText($"[color=#{textHex}]{EscapeBbcode(message)}[/color]\n");
        ScrollToBottom();
    }
