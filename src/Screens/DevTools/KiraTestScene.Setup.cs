...
public partial class KiraTestScene
{
	private void GetDebugControls()
	{
		...
		_kiraPortraitFrame = GetNode<PanelContainer>(D + "KiraPortraitMargin/KiraPortraitFrame");
		_kiraPortrait = GetNode<TextureRect>(D + "KiraPortraitMargin/KiraPortraitFrame/KiraPortrait");

		// Create overlay TextureRect for crossfade transitions
		_kiraPortraitNext = new TextureRect
		{
		...
		_kiraPortrait.AddSibling(_kiraPortraitNext);
		_kiraPortraitNext.SetAnchorsAndOffsetsPreset(LayoutPreset.FullRect);
		// LlmToggle removed
	...
	private void ApplyAccentColors()
	{
		...
		string[] headers = { "KiraHeader", "ModelHeader", "InferenceHeader", "PromptHeader", "ContextHeader", "OutputHeader", "ProactiveHeader", "MemoryHeader" };
		...
		_levelSelector.TooltipText = "KIRA evolution level. Levels 0-2 use scripted responses. Level 3+ activates the LLM.";
		_emotionDisplay.TooltipText = "Current KIRA emotion detected from the last response. Parsed from [EMOTION] tags. Read-only.";
		...
		_kiraPortraitFrame.AddThemeStyleboxOverride("panel", frameStyle);

		// Load emotion textures
		foreach (var emotion in KiraResponse.ValidEmotions)
		{
			var path = $"res://src/Assets/Textures/Kira/kira-{emotion.ToLowerInvariant()}-v2.png";
		...
		SetKiraEmotion("NEUTRAL");

		// Resize portrait height to maintain 3:4 aspect ratio
		_kiraPortraitFrame.Resized += UpdatePortraitHeight;
		...
		_threatSelector.TooltipText = "Simulated threat level. Injected into KIRA's context.";
		_sectorInput.TooltipText = "Current sector coordinates. Injected into KIRA's context.";
		_factionInput.TooltipText = "Player's faction alignment. Injected into KIRA's context.";
	}
	...
	private void ApplySliderAccents()
	{
		(HSlider slider, Label value, string namePath, string tooltip)[] sliderRows =
		{
			...
			(_responseDelaySlider, _responseDelayValue, "ResponseDelay", "Artificial delay before KIRA responds (scripted mode). Simulates processing time for immersion."),
			...
			(_maxTokensSlider, _maxTokensValue, "MaxTokens", "Maximum length of KIRA's response in tokens (~4 chars each). Higher = longer answers but slower."),
			...
			(_hullSlider, _hullValue, "Hull", "Station hull integrity percentage. Injected into KIRA's context."),
			(_powerSlider, _powerValue, "Power", "Station power output in kW. Injected into KIRA's context."),
			(_dronesActiveSlider, _dronesActiveValue, "DronesActive", "Number of drones currently deployed. Injected into KIRA's context."),
			(_dronesIdleSlider, _dronesIdleValue, "DronesIdle", "Number of idle drones in hangar. Injected into KIRA's context."),
			(_storageSlider, _storageValue, "Storage", "Station storage capacity usage %. Injected into KIRA's context."),
			(_proactiveIntervalSlider, _proactiveIntervalValue, "ProactiveInterval", "Seconds between proactive checks. Lower = more frequent KIRA messages."),
		};
	...
	private void SetupAllControls()
	{
		...
		// KIRA
		foreach (var level in Enum.GetValues<KiraLevel>())
	...
	{
		var data = GD.Load<Resources.AI.KiraLevelData>(path);
		if (data != null)
		{
			_kira.RegisterLevelData(data);
	...
	private void LoadConfigToUI()
	{
		...
		// LlmToggle removed
		_levelSelector.Selected = _config.KiraLevel;
	...
	private void SaveConfig()
	{
		...
		_config.KiraLevel = _levelSelector.Selected;
