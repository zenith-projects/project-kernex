...
/// <summary>
/// AXIA debug/test scene with chat panel (left) and debug controls (right).
/// Theme: src/Assets/Themes/Axia/axia_debug_theme.tres; glass shader provides frosted background.
/// </summary>
public partial class AxiaTestScene : Control
{
	...
	private AxiaDebugConfig _config;
	private bool _loadingConfig;

	// Memory + Proactive
	private AgentMemoryStore _memoryStore; // convenience ref → _axia.MainAgent.Memory
	private AxiaProactiveEngine _proactiveEngine;
	...
	private TextureRect _axiaPortrait;
	private TextureRect _axiaPortraitNext;
	private PanelContainer _axiaPortraitFrame;
	...
	{
		_axiaPanel = GetNode<AxiaPanel>("ScreenMargin/HSplitContainer/AxiaPanel");

		_config = AxiaDebugConfig.LoadOrCreate();
		...
		_memoryStore = (AgentMemoryStore)_axia.MainAgent.Memory;

		// Initialize proactive engine with sub-agent
		_proactiveEngine = new AxiaProactiveEngine();
		_proactiveEngine.SetAgent(_proactiveAgent);
		_proactiveEngine.ProactiveMessageReady += OnProactiveMessage;

		_axiaPanel.MessageSubmitted += OnMessageSubmitted;
		_axiaPanel.SetLevel((AxiaLevel)_config.AxiaLevel);

		// Restore recent chat messages from memory
		RestoreChatFromMemory();
		SetAxiaEmotion(_config.CurrentEmotion ?? "NEUTRAL");

		CreateDownloadOverlay();
		_axiaPanel.FocusInput();

		Log("AXIA Test Scene ready. Config loaded.");
	...
	{
		// Overlay covers the AxiaPanel (left side)
		_downloadOverlay = new PanelContainer();
		_axiaPanel.AddChild(_downloadOverlay);
		...
		var subtitle = new Label { Text = "AXIA needs these models to function. This only happens once.", HorizontalAlignment = HorizontalAlignment.Center };
