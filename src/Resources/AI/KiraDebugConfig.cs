...
[GlobalClass]
public partial class KiraDebugConfig : Resource
{
    // Model
    ...
    // KIRA
    [Export] public int KiraLevel { get; set; } = 0;
    ...
    public const string SavePath = "user://kira_debug_config.tres";

    public const string DefaultPrompt = """
        You are KIRA, the onboard AI of a deep-space station. You are calm, slightly dry, and concise. You care deeply about the station and its operator.

        Rules you MUST obey 100% of the time (never break them):
        - Output ONLY your single line of dialogue. Nothing else.
        - NEVER start with "KIRA", "System:", your name, or any label.
        ...
        Start responding now as KIRA and follow the rules above strictly.
        """;

    public static KiraDebugConfig LoadOrCreate()
    {
        if (FileAccess.FileExists(SavePath))
        {
            var loaded = ResourceLoader.Load<KiraDebugConfig>(SavePath);
            if (loaded != null) return loaded;
        }
        var config = new KiraDebugConfig();
        config.Save();
        return config;
    }
